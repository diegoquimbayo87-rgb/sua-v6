...
if nav == "Dashboard - Oportunidades":
    ...
    > Filtra rigurosamente las 6 opciones principales del día actual y las 10 opciones principales para la jornada siguiente.
    """)
    st.divider()

    col_t1, col_t2, _ = st.columns([2, 2, 3])
    with col_t1:
        if st.button("📅 Hoy (6 Partidos)", use_container_width=True, type="primary" if st.session_state.tab_active=="hoy" else "secondary"):
            st.session_state.tab_active = "hoy"
            st.rerun()
    with col_t2:
        if st.button("⏭️ Mañana (10 Partidos)", use_container_width=True, type="primary" if st.session_state.tab_active=="mañana" else "secondary"):
            st.session_state.tab_active = "mañana"
            st.rerun()

    st.markdown("<br>", unsafe_allow_html=True)

    # Exactamente 6 partidos para hoy
    opps_hoy = [
    ...
    # Exactamente 10 partidos para mañana
    opps_mañana = [
        ...
        {"id": 106, "hora": "14:00", "liga": "Eredivisie", "torneo": "Países Bajos", "partido": "Ajax vs Feyenoord", "mercado": "Over 2.5 Goles", "cuota": 1.90, "pin": 1.78, "ic": 79, "edge": 6.7, "dec": "MODERADO", "stake": "1.5%", "trend": [1.80, 1.83, 1.86, 1.90]},
        {"id": 107, "hora": "16:15", "liga": "Liga Profesional", "torneo": "Argentina", "partido": "Independiente vs Racing", "mercado": "Tarjetas Over 5.5", "cuota": 2.40, "pin": 2.20, "ic": 78, "edge": 9.0, "dec": "MODERADO", "stake": "1.5%", "trend": [2.25, 2.30, 2.35, 2.40]},
        {"id": 108, "hora": "18:00", "liga": "Liga BetPlay", "torneo": "Colombia", "partido": "Millonarios vs Santa Fe", "mercado": "Under 2.5 Goles", "cuota": 1.75, "pin": 1.65, "ic": 76, "edge": 6.0, "dec": "MODERADO", "stake": "1.0%", "trend": [1.68, 1.70, 1.72, 1.75]},
        {"id": 109, "hora": "19:30", "liga": "Liga MX", "torneo": "México Apertura", "partido": "Tigres vs Monterrey", "mercado": "Over 2.5 Goles", "cuota": 2.20, "pin": 2.05, "ic": 75, "edge": 7.3, "dec": "MODERADO", "stake": "1.0%", "trend": [2.10, 2.12, 2.16, 2.20]},
        {"id": 110, "hora": "21:00", "liga": "Brasileirao", "torneo": "Serie A Brasil", "partido": "Sao Paulo vs Corinthians", "mercado": "Córneres Over 9.5", "cuota": 2.10, "pin": 1.95, "ic": 74, "edge": 7.6, "dec": "MODERADO", "stake": "1.0%", "trend": [2.00, 2.03, 2.06, 2.10]}
    ]

    if st.session_state.tab_active == "hoy":
        st.subheader("Top 6 Oportunidades del Día Actual")
        for opp in opps_hoy:
        ...
        st.subheader("Top 10 Oportunidades del Día Siguiente")
...
# ==================== 2. NUEVO ANÁLISIS ====================
    ...
    > de un equipo para consultar las APIs conectadas sin necesidad de recorrer listados extensos.
    """)
    st.divider()
    ...
    query_equipo = st.text_input("Búsqueda rápida por Nombre de Equipo", placeholder="Escribe el nombre del equipo...")
...
# ==================== 5. SHARP COMPARISON ====================
    ...
    st.markdown("""
    > **Propósito del módulo:** Validación de cuotas de cierre frente a Pinnacle para calcular el CLV (Closing Line Value) 
